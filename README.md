...
(Do **not** add a root `main.py` — Vercel reserves that name. The HTTP entry for deploys is [`api/index.py`](api/index.py) (FastAPI `app`, pipeline at **`/cron`** on the deployment root); the pipeline CLI stays in [`pipeline.py`](pipeline.py).)
...
3. **Function duration:** we do **not** set `functions` in `vercel.json` — Python/FastAPI builds often don’t match those glob keys and Vercel fails with [unmatched function pattern](https://vercel.link/unmatched-function-pattern). Set **max duration** in the Vercel dashboard: Project → **Settings** → **Functions** (Hobby up to **300s**, Pro up to **800s**). `/cron` runs `PIPELINE_MODE=auto`, which rotates smaller core/tracking/reporting jobs; use a manual `full` run only when the timeout budget can handle it.
